  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div className="h-full flex flex-col bg-navy-900/95 backdrop-blur-2xl border border-slate-700/50 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50 bg-gradient-to-r from-blue-500/10 to-purple-500/10 flex-shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  {!minimized && (
                    <p className="text-[10px] text-slate-400">Montgomery Command Center</p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="p-1.5 text-slate-400 hover:text-red-400 rounded-lg hover:bg-slate-800/50 transition-colors"
                  title="Close"
                >
            ...
            {!minimized && (
              <>
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
                  {messages.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-center px-4">
                      ...
                      <p className="text-sm text-slate-400 mb-1">How can I help?</p>
                      <p className="text-[11px] text-slate-500 mb-4">Ask anything about Montgomery's data</p>
                      <div className="flex flex-col gap-1.5 w-full">
                        {quickQuestions.map((q, i) => (
                          <button
                            key={i}
                            onClick={() => setInput(q)}
                            className="text-left px-3 py-2 rounded-xl bg-slate-800/40 border border-slate-700/30 text-[11px] text-slate-400 hover:text-slate-200 hover:border-blue-500/30 transition-all"
                          >
                            {q}
                          </button>
                        ))}
                  ...
                  {action.loading && (
                    <div className="flex gap-2">
                      ...
                      <div className="bg-slate-800/50 border border-slate-700/30 rounded-2xl px-3 py-2 flex items-center gap-1.5">
                ...
                {/* Input */}
                <form onSubmit={handleSend} className="flex gap-2 p-3 border-t border-slate-700/50 flex-shrink-0">
                  <input
                    ref={inputRef}
                    type="text"
                    value={input}
                    ...
                    className="flex-1 px-3 py-2 bg-slate-800/50 border border-slate-700/50 rounded-xl text-xs text-slate-300 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500/40 focus:border-blue-500/40 transition-all"
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-[90] w-14 h-14 rounded-full shadow-2xl shadow-blue-500/30 flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 ${
          open
            ? 'bg-slate-800 border border-slate-700/50 text-slate-400 hover:text-white'
